...
from GAN.examples.CONV_CUB.model import Generator,Discriminator
from GAN.utils import weight_filler
import torch
...
modelpath = '/data/schirrmr/hartmank/data/GAN/models/GAN_Progressive/%s/'%('PAPERFIN4_'+subj_names[subj_ind]+'_FFC4h_WGAN_adaptlambclamp_CONV_CUB_10l_run%d'%task_ind)
...
generator.train_init(alpha=lr,betas=(0.,0.99),eps_center=0.001,
...
for i_block in range(i_block_tmp,n_blocks):
	...
	train_tmp = discriminator.model.downscale_to_block(Variable(torch.from_numpy(train).cuda(),volatile=True),discriminator.model.cur_block).data.cpu()
	...
	for i_epoch in range(i_epoch_tmp,block_epochs[i_block]):
		...
		if i_epoch%100 == 0:
			...
			plt.subplot(3,2,1)
			plt.plot(np.asarray(losses_d)[:,0],label='Loss Fake')
			plt.plot(np.asarray(losses_d)[:,1],label='Loss Real')
